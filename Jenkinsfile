            ...
            steps {
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-key']]) {
                        ...
                        // 2. PowerShell 스크립트로 application.properties 파일 업데이트
                        bat """
                        powershell -Command "\$kafkaUrl = '${kafka_service_url}'; (Get-Content 'E:\\docker_Logi\\logi_react_back_cloud\\src\\main\\resources\\application.properties') -replace 'spring.kafka.bootstrap-servers=.*', 'spring.kafka.bootstrap-servers=a477a61afcf2545e6a7f4efef0902afe-1550994357.ap-northeast-2.elb.amazonaws.com:9092' | Set-Content 'E:\\docker_Logi\\logi_react_back_cloud\\src\\main\\resources\\application.properties';"
                        """
                    }
                }
            }
